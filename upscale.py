            ...
            for idx, img_path in enumerate(images, 1):
                ...
                if self.seamless == SeamlessOptions.tile:
                    img = cv2.copyMakeBorder(
                        img, 16, 16, 16, 16, cv2.BORDER_WRAP)
                    ...
                    img = cv2.copyMakeBorder(
                        img, 16, 16, 16, 16, cv2.BORDER_REPLICATE)
        ...
        if (
            ...
            if self.binary_alpha:
                ...
                _, alpha = cv2.threshold(
                    alpha, threshold, 1, cv2.THRESH_BINARY)
                output[:, :, 3] = alpha
...
def main(
    ...
    output: Path = typer.Option(
        Path("output"), "--output", "-o", help="Output folder"),
    reverse: bool = typer.Option(
        False, "--reverse", "-r", help="Reverse Order"),
